...
import { MOCA, getCognitiveSeverity, getDomainScore } from "@/lib/cognitiveScreenings";

export const dynamic = "force-dynamic";

export default async function MoCADetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const severity = getCognitiveSeverity(score, MOCA);
  const answers: Record<string, number> = screening.answers || {};
  const educationBonus = (answers.education_bonus ?? 0) === 1;
  ...
  return (
    <div className="max-w-3xl space-y-5 pb-10">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-slate-900">MoCA Results</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="text-6xl font-bold text-slate-900 mt-1">
              {score}<span className="text-xl font-normal text-slate-400">/{MOCA.maxScore}</span>
            </div>
            {educationBonus && <div className="text-xs text-indigo-600 mt-1 font-medium">Includes +1 education adjustment (≤12 yrs schooling)</div>}
          </div>
        ...
        <div className="bg-slate-100 rounded-full h-4 overflow-hidden mb-3">
          <div className={`h-4 rounded-full transition-all ${
            score >= 26 ? "bg-emerald-500" : score >= 18 ? "bg-amber-500" : score >= 10 ? "bg-orange-500" : "bg-red-500"
          }`} style={{ width: `${(score / MOCA.maxScore) * 100}%` }} />
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
          ...
          <div><div className="text-xs text-slate-400">Tool</div><div className="font-bold font-mono text-slate-900">MoCA</div></div>
        </div>
      </div>
      ...
      <div className="flex gap-3 pb-4">
        ...
        <Link href="/dashboard/screenings/moca/new" className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-500">
          Administer Again
        </Link>
      </div>
    </div>
