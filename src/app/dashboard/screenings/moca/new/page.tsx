...
import { MOCA, getCognitiveScore, getCognitiveSeverity, getDomainScore } from "@/lib/cognitiveScreenings";
...
function MoCAForm() {
  ...
  const allItems = MOCA.domains.flatMap(d => d.items);
  const answeredCount = allItems.filter(item => answers[item.id] !== undefined).length;
  const allAnswered = answeredCount === allItems.length;
  const totalScore = getCognitiveScore(answers, MOCA, educationBonus);
  const severity = getCognitiveSeverity(totalScore, MOCA);
  ...
  async function handleSubmit() {
    ...
    const res = await fetch("/api/screenings", {
      ...
      body: JSON.stringify({
        ...form,
        tool: "moca",
        answers: { ...answers, education_bonus: educationBonus ? 1 : 0 },
        total_score: totalScore,
        severity_label: severity.label,
      }),
    });
    const data = await res.json();
    if (res.ok) router.push(`/dashboard/screenings/moca/${data.screening.id}`);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">MoCA — Montreal Cognitive Assessment</h1>
          <p className="text-slate-500 text-sm mt-0.5">Clinician-administered · 30 points · Validated for MCI and dementia screening</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
        ...
        <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3">
          <input type="checkbox" id="edu_bonus" checked={educationBonus} onChange={e => setEducationBonus(e.target.checked)}
            className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
          <label htmlFor="edu_bonus" className="text-sm font-medium text-indigo-800 cursor-pointer">
            Add 1-point education adjustment (client has ≤12 years of formal education)
          </label>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="flex-1 bg-slate-100 rounded-full h-2 overflow-hidden">
              <div className="bg-indigo-500 h-2 rounded-full transition-all" style={{ width: `${pctComplete}%` }} />
            </div>
            <span className="text-xs font-semibold text-slate-600">{pctComplete}%</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Running score: <span className="font-bold text-slate-900">{totalScore}/{MOCA.maxScore}{educationBonus ? "+1" : ""}</span></span>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-5 py-4">
        <p className="text-sm font-semibold text-indigo-900 mb-1">Administration Instructions</p>
        <p className="text-sm text-indigo-800">Administer each item in order. Score items as directed below. The MoCA is a clinician-scored instrument — expand each item for detailed administration notes.</p>
        <p className="text-xs text-indigo-600 mt-2">Standard cutoff: ≥26 = normal cognition. Add 1 point if ≤12 years of formal education (checked above).</p>
      </div>

      {/* Domain-based scoring */}
      <div className="space-y-4">
        {MOCA.domains.map(domain => {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        ...
        <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3}
          className={inputClass + " resize-none"} placeholder="Behavioral observations, language barriers, physical limitations, referral plan..." />
      </div>

      {/* Results preview */}
      {allAnswered && (
        <div className="border-2 border-indigo-200 rounded-2xl p-5 bg-white">
          <div className="flex items-center justify-between mb-2">
            <div className="font-bold text-slate-900 text-lg">MoCA Results</div>
            <div className="text-3xl font-bold text-slate-900">{totalScore}<span className="text-sm font-normal text-slate-400">/{MOCA.maxScore}</span></div>
          </div>
          ...
          {educationBonus && (
            <p className="text-xs text-indigo-600 mt-2 font-medium">* Score includes +1 education adjustment for ≤12 years formal schooling.</p>
          )}
        </div>
      )}
      ...
      <div className="flex gap-3 justify-end pb-6">
        ...
        <button onClick={handleSubmit} disabled={!allAnswered || !form.client_id || saving}
          className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-500 disabled:opacity-50">
          {saving ? "Saving..." : "Save MoCA →"}
        </button>
      </div>
    </div>
  );
...
export default function MoCANewPage() {
  return <Suspense fallback={<div className="p-8 text-slate-400">Loading...</div>}><MoCAForm /></Suspense>;
}
